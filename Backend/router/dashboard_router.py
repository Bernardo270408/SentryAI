...
def get_dashboard_stats():
    ...
    if not messages:
        ...
        top_categories_list = [
            k for k, v in most_common
        ]  # Lista de nomes: ['Direito Civil', 'Trabalhista'...]
    ...
    top_doubts = (
        analyze_user_doubts(user_msgs_text[:15])
        if user_msgs_text
        else ["Nenhuma dúvida registrada."]
    )
    ...
    history_data = [
        {
            "id": c.id,
            "action": c.name or "Nova Conversa",
