...
def verify_email():
    ...
    if user.is_verified:
        # Se já verificado, apenas retorna sucesso (ou gera token para logar direto)
        token = generate_token(user)
        return jsonify({'message': 'Usuário já verificado', 'token': token, 'user': user.to_dict()}), 200
...
def google_login():
    ...
    try:
        ...
        if not user:
            ...
            # Se já existe, vincula o google_id e marca como verificado (confiança no Google)
            changed = False
