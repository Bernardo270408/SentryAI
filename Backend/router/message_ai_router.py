...
@message_ai_bp.route('/<int:message_id>/rate', methods=['POST'])
@token_required
def rate_ai_message(message_id):
    current_user = request.user
    message = AIMessageDAO.get_message_by_id(message_id)
    ...
    try:
        rated_message = AIMessageDAO.rate_message(message_id, rating, feedback)
    except ValueError as ve:
        return jsonify({'error': str(ve)}), 400

    if not rated_message:
        return jsonify({'error': 'Rating failed'}), 400

    return jsonify(rated_message.to_dict())


@message_ai_bp.route('/rate', methods=['GET'])
@token_required
def get_rated_ai_messages():
    current_user = request.user
    if not current_user.is_admin:
        return jsonify({'error': 'Permission denied'}), 403

    messages = AIMessageDAO.get_messages_with_ratings()
    return jsonify([m.to_dict() for m in messages])

@message_ai_bp.route('/<int:message_id>/feedback', methods=['POST'])
@token_required
def feedback_ai_message(message_id):
    current_user = request.user
    message = AIMessageDAO.get_message_by_id(message_id)
    ...
    rated_message = AIMessageDAO.rate_message(message_id, message.rating, feedback)

    if not rated_message:
        return jsonify({'error': 'Feedback update failed'}), 400

    return jsonify(rated_message.to_dict())

@message_ai_bp.route('/<int:message_id>/feedback', methods=['GET'])
@token_required
def get_feedback_ai_message(message_id):
    current_user = request.user
    message = AIMessageDAO.get_message_by_id(message_id)
    ...
    return jsonify({'feedback': message.feedback})
