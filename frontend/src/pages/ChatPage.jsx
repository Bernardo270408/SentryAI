...
// ------------------------------------------------------------------
// NOVO COMPONENTE: MODAL DE CRIAÇÃO DE CHAT
// ------------------------------------------------------------------
...
// COMPONENTE para a tela de boas-vindas (Empty State)
function EmptyChatState({ onOpenModal }) { // Alterado de onCreateChat para onOpenModal
    const features = [
    ...
    return (
        <div className="empty-state">
            ...
            {/* Chama o handler para abrir o modal */}
...
export default function ChatPage() {
    ...
    // FUNÇÃO ATUALIZADA: É chamada pelo Modal com o nome fornecido
    async function handleCreateChat(name) {
        // O prompt foi removido daqui e a função agora recebe o nome (name)
        try {
    ...
    // HANDLER para apenas abrir o Modal
    ...
    async function handleSend(e) {
        ...
        setAttachedFile(null); // Limpa o anexo após o envio
        setSending(true);
        ...
        try {
            const es = api.streamChatMessage({
                message: contentWithAttachment, // Envia o conteúdo com o anexo para a API
                // ... (onChunk, onEnd, onError mantidos)
                onChunk: (token) => {
                    setMessages((prev) => {
                        const copy = [...prev];
    ...
    return (
        <div className="chat-root">
            ...
            <aside className={`fixed-sidebar glass ${!isSidebarOpen ? "collapsed" : ""}`}>
                ...
                {/* BOTÃO NOVA PERGUNTA - CHAMA O MODAL */}
                <button className="btn-new" onClick={handleOpenCreateChatModal}>
                    <FiPlus /> Novo Chat
                </button>

                {/* BUSCA DE CHAT */}
                {isSidebarOpen && ( // Mostra a busca apenas se a sidebar estiver aberta
                ...
                {/* Conteúdo Principal da Sidebar (Lista de Chats) */}
                {isSidebarOpen && (
                ...
                {/* MARCA D'ÁGUA VERTICAL QUANDO COLAPSADA */}
                {!isSidebarOpen && (
                    <div className="sidebar-collapsed-footer">
                        <GptIcon />
                        <div className="vertical-text">
                            Alimentado por GPT-5
                        </div>
                    </div>
                )}

                {/* Rodapé quando aberta */}
                {isSidebarOpen && (
            ...
            <main className={`chat-main ${!isSidebarOpen ? "expanded" : ""}`}>
                ...
                <div className="floating-input-wrap">
                    ...
                    <form className="floating-pill glass-pill" onSubmit={handleSend} onKeyDown={handleKeyDown}>
                        ...
                        {/* INPUT DE ARQUIVO (OCULTO, APENAS ÍCONE VISÍVEL) */}
